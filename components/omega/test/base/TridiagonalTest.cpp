...
int testCorrectness(int NBatch, int NRow) {
   ...
   Kokkos::parallel_for(
       Policy, KOKKOS_LAMBDA(const TeamMember &Member) {
          TriDiagSolver::solve(Member, DL, D, DU, AX);
       });
...
int testDiffusionCorrectness(int NBatch, int NRow) {
   ...
   Kokkos::parallel_for(
       Policy, KOKKOS_LAMBDA(const TeamMember &Member) {
          TriDiagDiffSolver::solve(Member, G, H, AX);
       });
...
int tridiagonalTest() {
   ...
   for (int NBatch : {1, 2, 4, 5, 11, 33, 102}) {
      for (int NRow : {1, 2, 3, 4, 5, 6, 11, 17, 64, 100}) {
...
void testPerformance(int NBatch, int NRow, int NRep) {
   ...
   Kokkos::parallel_for(
       Policy, KOKKOS_LAMBDA(const TeamMember &Member) {
          TriDiagSolver::solve(Member, DL, D, DU, X);
       });
...
void testDiffusionPerformance(int NBatch, int NRow, int NRep) {
   ...
   Kokkos::parallel_for(
       Policy, KOKKOS_LAMBDA(const TeamMember &Member) {
          TriDiagDiffSolver::solve(Member, G, H, X);
       });
